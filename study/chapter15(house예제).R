...
1. 통계청 출산 데이터(시계열, 회귀)
2. 집 값 데이터(boston집값 데이터는 x(흑인 거주비율 들어있어서),
           캘리포니아 많이 씀, 대표적 회귀)
...
#후처리
house$mean_bedromms = house$total_bedrooms / house$households
...
## 전처리(상식을 사용해서 가정에 대한 데이터를 별도로 분리)(ocean_proximity 값 변경)
...
test
#iris
#x <- iris$Petal.Length + iris$Sepal.Length + iris$Sepal.Width + iris$Petal.Width
#summary(x)
#y <-iris$Species
#summary(y)
