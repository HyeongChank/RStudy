...
library(wordcloud)
facebook <- file("./data/facebook_bigdata.txt", 
                 encoding = "UTF-8")
facebook_data <- readLines(facebook)
head(facebook_data)
text_data <- c(facebook_data)
text_data
word_freq <- table(unlist(strsplit(text_data, " " , fixed = TRUE)))
top_words <- head(sort(word_freq, decreasing = TRUE), 30)
top_words
wordcloud(words = names(top_words), freq = top_words, min.freq = 1,
          scale = c(4,0.5),
          random.order = FALSE, rot.per = 0.35,
          colors = brewer.pal(8,"Dark2"), shape ="circle")
